...
class DatafieldDetect:
    def __init__(self, detect_data, rule_data):
        ...
        self.rule_dict = {}
        for canid in self.canIDset:
            self.rule_dict[canid] = []
            for index, single_rule in self.rule_data.loc[[canid], :].iterrows():
                tmp_dict = {'type': single_rule['type'], 'begin_loc': single_rule['begin_loc'],
                            'length': single_rule['length'], 'value_list': single_rule['value_list']}
                self.rule_dict[canid].append(tmp_dict)
    ...
    def line_detect(self, row):
        ...
        for single_rule in self.rule_dict[row['can_id']]:
            ...
            if single_rule['type'] == CHECKSUM_TAG:
                ...
                if not sen_min <= sen_data <= sen_max:
                    self.add_to_deviant(row, SENSOR_OUT_RANGE, single_rule['begin_loc'])
