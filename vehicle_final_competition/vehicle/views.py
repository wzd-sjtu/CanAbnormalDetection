...
def _parse_sequence(request):
    if global_data:
        ...
        return render(request, 'parse/sequence.html', {'targetChoics': allCanIdList})


def _parse_lstm(request):
    if global_IDs:
        pf.set_detect_seq_lstm(global_IDs)
        return render(request, 'parse/sequence.html', {'result': "训练成功", 'targetChoics': allCanIdList})
    else:
        return render(request, 'parse/sequence.html', {'targetChoics': allCanIdList})
...
def _parse_datafield(request):
    ...
    return render(request, 'parse/datafield.html', {'data_rule': show_rule_dataframe, 'cluster_array': cluster_dict_array, 'targetChoics': allCanIdList})
    ...
    if request.is_ajax():
        ...
        if chose_id == 'undefined':
            ...
            for i in range(0, num):
                ...
                target_dict[description_loc+str(i)] = str(tmp.iloc[i][description_loc])
...
def _construct_fuzz(request):
    return render(request, 'construct/construct_fuzz.html', {'targetChoics':allCanIdList})
def fuzz_attack(request):
    if request.is_ajax():
        # print(request.POST)
        # formdata.append("chose_id", $("#chose_id").val());
        # formdata.append("attack_exist_time", $("#attack_exist_time").val());
        ...
        # def insert_attack(self, id, normal_T, ratio, exist_time):
        attackCreateExample.fuzz_attack(str(chose_id), float(chose_attack_cycle_time), 1, float(chose_attack_exist_time))
        tmp = pd.read_csv('./CanConstruct/src/attackDescription/myDescription.csv')
        # 获得log信息，是可信的
        target_dict = {}
        num = tmp.shape[0]
...
def attack_make(request):
    ...
    if request.is_ajax():
        ...
        if chose_attack_type == "注入攻击":
            attackCreateExample.insert_attack(str(chose_id), float(chose_attack_cycle_time), 1,
                                              float(chose_attack_exist_time))
        elif chose_attack_type == "重放攻击单ID":
            attackCreateExample.reput_attack_SingleId(str(chose_id), float(chose_attack_exist_time))
        elif chose_attack_type == "重放攻击多ID":
            attackCreateExample.reput_attack_AllData(float(chose_attack_exist_time))
        elif chose_attack_type == "Fuzz攻击":
            attackCreateExample.fuzz_attack(str(chose_id), float(chose_attack_cycle_time), 1,
                                            float(chose_attack_exist_time))
        elif chose_attack_type == "欺骗攻击":
            if chose_datafield_attack_method == "随机修改":
                attackCreateExample.changedatafield_attack_randomly(str(chose_id), float(chose_attack_exist_time))
            elif chose_datafield_attack_method == "定值或多值字段修改":
                attackCreateExample.changedatafield_attack_const_or_multivalue(str(chose_id),float(chose_attack_exist_time))
            elif chose_datafield_attack_method == "传感器字段修改":
                # 具体的攻击逻辑存储在后台的类里面，并不存在于这里的
                tyty = 0
                if chose_sensor_attack_type == "最大值":
                    tyty = 0
                elif chose_sensor_attack_type == "最小值":
                    tyty = 1
                elif chose_sensor_attack_type == "随机值":
                    tyty = 2
                elif chose_sensor_attack_type == "冗余小选项":
                    tyty = 3
                attackCreateExample.changedatafield_attack_sensro(str(chose_id), float(chose_attack_exist_time), tyty)
        tmp = pd.read_csv('./CanConstruct/src/attackDescription/myDescription.csv')
...
def see_anormal_data(request):
    target_dict={}

    if request.is_ajax():
        detect_data = pd.read_csv('./CANConstruct/src/attackDescription/myDescription.csv', index_col=0)
        num = detect_data.shape[0]

        target_dict['size'] = str(num)
        for i in range(0, num):
            time_loc = "time" + str(i)
            target_dict[time_loc] = str(detect_data.iloc[i]['time'])
            can_id_loc = "can_id" + str(i)
            target_dict[can_id_loc] = str(detect_data.iloc[i]['can_id'])
            data_loc = "data_in_hex" + str(i)
            target_dict[data_loc] = str(detect_data.iloc[i]['data_in_hex'])
            description_loc = "description" + str(i)
            target_dict[description_loc] = str(detect_data.iloc[i]['description'])
