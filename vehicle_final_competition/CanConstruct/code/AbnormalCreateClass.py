...
class AttackCreate:
    ...
    # Fuzz攻击
    def fuzz_attack(self, id, normal_T, ratio, exist_time):
        # 参数意义注解
        # fuzz攻击是注入报文的数据域任意修改的结果
        # id 报文id
        # normal_T 经过序列分析，得知这个报文的周期
        # ratio 注入周期是报文周期的倍数
        # exist_time 注入报文攻击的持续时间
        ...
        for i in range(1, self.sourceDataSnippet.shape[0] - 1):
            ...
            if begin_time > tmp_origin_data.iloc[i]['time'] and begin_time < tmp_origin_data.iloc[i + 1]['time']:
                ...
                # 在这里df的数据字段并没有进行复杂的修改
                df3['can_id'] = id
                df3['anormal'] = 10  # 证明是fuzz攻击的一种
                # 这里是注入攻击
                descriptionTmp = "Pre data is " + df3['data_in_hex']

                # 完成基础的数据替换
                data_replace_in_binary = ""
                for zyh in range(0, 64):
                    if random.random()<0.5:
                        data_replace_in_binary = data_replace_in_binary + "0"
                    else:
                        data_replace_in_binary = data_replace_in_binary + "1"
                df3['data_in_binary'] = data_replace_in_binary
                df3['data_in_hex'] = binary_str_to_hex_str(data_replace_in_binary)

                descriptionTmp = descriptionTmp + " Now data is " + df3['data_in_hex']


                # def updateBasicInformation(self, type, id, time, description, data_in_binary):
                self.descriptionStruct.updateBasicInformation(10, df3['can_id'],
                                                              df3['time'],
                                                              descriptionTmp,
                                                              df3['data_in_binary']) # 在这里存储的是二进制数据
